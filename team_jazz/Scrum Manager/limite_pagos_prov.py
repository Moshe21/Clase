...
def abrir_limites_pagos_prov():
    def establecer_limite():
        ...
        sql = f"UPDATE limite_pagos SET monto = {valor} WHERE destinatario = 'proveedores'"
        cursor.execute(sql)
        conexion.commit()
        conexion.close()
    ...
    ventana.title("Scrum Manager | Limite de pago a Proveedores")
